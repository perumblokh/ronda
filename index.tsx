...
const AttendanceForm: React.FC<{
  ...
  const now = new Date();
  const cutoffHour = 6; // Shift ends at 6 AM. Times before this belong to the previous day.

  const effectiveDate = new Date(now);
  if (now.getHours() < cutoffHour) {
    // It's early morning, so this shift belongs to the previous calendar day.
    effectiveDate.setDate(now.getDate() - 1);
  }

  const dayOfWeek = effectiveDate.getDay();
  const todayDateString = effectiveDate.toISOString().split('T')[0];
